...
def test_flask_api_missing_field():
    from app.flask_api import app
    client = app.test_client()
    resp = client.post("/predict", json={"area": 5000})
...
def test_flask_accepts_binary_integers():
    """API should accept 0/1 as well as yes/no strings."""
    from app.flask_api import app
    client = app.test_client()
    payload = {
        'area': 4000, 'bedrooms': 3, 'bathrooms': 2, 'stories': 1,
        'mainroad': 1, 'guestroom': 0, 'basement': 0,
        'hotwaterheating': 0, 'airconditioning': 1, 'parking': 1,
        'prefarea': 0, 'furnishingstatus': 'furnished',
    }
    resp = client.post("/predict", json=payload)
    assert resp.status_code == 200
